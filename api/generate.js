export default async function handler(req, res) {
  ...
  const { product, features, price, audience } = req.body || {}
  if (!product) {
...
[{"title":"标题","content":"正文内容","tags":["标签1","标签2","标签3"]}]`
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(500).json({ error: data.error?.message || 'AI 服务异常' })
    }

    const raw = data.choices[0].message.content
    const cleaned = raw.replace(/```json\n?/g, '').replace(/```/g, '').trim()

    return res.status(200).json({ posts: JSON.parse(cleaned) })
  } catch (e) {
